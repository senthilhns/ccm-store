...
c7n validate find-no-search-or-indexing-last-14-days.yaml # checks policy YAML is valid  or not
c7n run -s /tmp/output_dir ./find-no-search-or-indexing-last-14-days.yaml # runs the policy and writes results to /tmp/output_dir
```

*** sample output just for testing SearchRate < 10000 and IndexingRate < 10000 was tested
```json
[
  {
    ...
    "c7n.metrics": {
      "AWS/ES.SearchRate.Average.14": [
        {
          "Timestamp": "2025-10-21T14:03:00+00:00",
          "Average": 0.028409090909090908,
          "Unit": "Count"
        }
      ],
      "AWS/ES.IndexingRate.Average.14": [
        {
          "Timestamp": "2025-10-21T14:03:00+00:00",
          "Average": 0.011363636363636364,
          "Unit": "Count"
        }
      ]
    }
...
```bash
jq -r '.[] | "\(.DomainName) | \(.ElasticsearchVersion) | \(.ElasticsearchClusterConfig.InstanceType) | \(.ElasticsearchClusterConfig.InstanceCount) | \(.EBSOptions.VolumeType) | \(.EBSOptions.VolumeSize) | \(.c7n.metrics["AWS/ES.SearchRate.Average.14"][0].Average) | \(.c7n.metrics["AWS/ES.IndexingRate.Average.14"][0].Average)"' /tmp/output_dir/find-no-search-or-indexing-last-14-days/resources.json
os23-rltst | OpenSearch_1.3 | t3.small.elasticsearch | 2 | gp3 | 10 | null | null
```